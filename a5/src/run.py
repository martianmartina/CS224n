...
if args.variant == 'vanilla':
    ...
    mconf.synthesizer = True # check model.py GPTConfig
...
if args.function == 'pretrain':
    ...
    # line 44-46: pretrain dataset is already loaded
    ...
    #         final_tokens=200*len(pretrain_dataset)*block_size
    #         num_workers=4
    if args.reading_params_path: #with pretraining
    ...
    model.load_state_dict(torch.load(args.reading_params_path))
    model = model.to(device) #all tensors should be on the same device
    correct = 0
    total = 0
