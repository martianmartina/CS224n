...
class SynthesizerAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        # B: num_batches
        # T: block_size (seq length) (parameter 'step' in sample when evaulating: 32)
        # C: total dim = nh * hs
        # nh: num_heads
        # hs: head_size
        ...
        att = F.relu(att)
        att = att @ self.w2[:,:T] + self.b2[:T] # (B, T, T, hs) * (hs, T)
        att = att.masked_fill(self.mask[:,:,:T,:T] == 0, -inf) # todo: just use float('-inf') instead?
        att = F.softmax(att, dim=-1)
        att = self.attn_drop(att)
        y = att @ v # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)
        y = y.transpose(1, 2).contiguous().view(B, T, C) # re-assemble all head outputs side by side
