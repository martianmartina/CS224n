...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        output,(final_h,final_c) = self.encoder(pack_padded_sequence(X,source_lengths))
        enc_hiddens = pad_packed_sequence(output)[0] #(seq_unpacked,len_unpacked)
        enc_hiddens = enc_hiddens.permute(1,0,2)
        concat_hidden = torch.cat((final_h[0],final_h[1]),dim=1)
        concat_cell = torch.cat((final_c[0],final_c[1]),dim=1)
        h_dec_init = self.h_projection(concat_hidden)
        c_dec_init = self.c_projection(concat_cell)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        a_t = torch.bmm(rearrange(enc_hiddens,'b l h -> b h l'),torch.unsqueeze(alpha_t,dim=2))
        # A better alternative:
        # a_t = torch.squeeze(torch.bmm(torch.unsqueeze(alpha_t, 1), enc_hiddens), dim=1)
        a_t = torch.squeeze(a_t,dim=2)
