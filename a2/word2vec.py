...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    y_est = softmax(np.dot(outsideVectors, centerWordVec))
    loss = -np.log(y_est[outsideWordIdx])

    len_vocab = outsideVectors.shape[0]
    y = np.zeros(len_vocab)
    y[outsideWordIdx] = 1
    gradCenterVec = np.dot(outsideVectors.T,(y_est-y))
    
    gradOutsideVecs = y_est.reshape(len_vocab,1) * np.tile(centerWordVec,(len_vocab,1))
    gradOutsideVecs[outsideWordIdx] = (y_est[outsideWordIdx]-1)*centerWordVec
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    for index in indices[1:]:
        loss -= np.log(sigmoid(-np.dot(outsideVectors[index].T,centerWordVec)))

    gradCenterVec = (sigmoid(np.dot(outsideVectors[outsideWordIdx].T,centerWordVec))-1)\
        *outsideVectors[outsideWordIdx]
    for index in indices[1:]:
        gradCenterVec += (1-sigmoid(-np.dot(outsideVectors[index].T,centerWordVec)))\
            *outsideVectors[index] 
    
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = \
        (sigmoid(np.dot(outsideVectors[outsideWordIdx].T,centerWordVec))-1)*centerWordVec
    for index in indices[1:]:
        gradOutsideVecs[index] += \
            (1-sigmoid(-np.dot(outsideVectors[index].T,centerWordVec)))*centerWordVec
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for o in outsideWords:
        _loss,_gradCenter,_gradOutside = word2vecLossAndGradient(
            centerWordVectors[word2Ind[currentCenterWord]],word2Ind[o],outsideVectors,dataset)
        loss += _loss
        gradCenterVecs[word2Ind[currentCenterWord]] += _gradCenter
